...
if __name__ == '__main__':
    ...
    revs, vocab, word_idx_map, maxlen, error_dict, error_idx = pickle.load(open(pickle_file, 'rb'))
    ...
    final_result_file = os.path.join('result', 'cged16_hsk_singlelabel_random_bilstm_crf.txt')
    with open(final_result_file, 'w') as my_file:
        for i in range(len(X_test)):
            ...
            for k in range(len(label)):
                if (label[k] == error_dict['R'] or label[k] == error_dict['S'] or \
                    label[k] == error_dict['M'] or label[k] == error_dict['W']) and error_flag == False:
                    error_flag = True
                    start_pos = k + 1
                    current_error = label[k]
                    is_correct = True

                if error_flag == True and label[k] != current_error and (label[k] != error_dict['R'] and label[k] != error_dict['S'] and \
                    label[k] != error_dict['M'] and label[k] != error_dict['W']) :
                    end_pos = k
                ...
                if error_flag == True and label[k] != current_error and (label[k] == error_dict['R'] or label[k] == error_dict['S'] or \
                    label[k] == error_dict['M'] or label[k] == error_dict['W']):
                    end_pos = k
                    my_file.write('%s, %d, %d, %s\n' % (sid, start_pos, end_pos, error_idx[current_error]))
